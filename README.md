...
>
> **The ADK agent** now runs live (its `GOOGLE_API_KEY` was fixed) and its two normal test cases pass — with one honest correction along the way: a test assertion had assumed `resolve_ticker` always fires as its own tool call, but a real trace showed the agent's model sometimes answers a famous ticker directly from its own knowledge instead, so that assertion was dropped as a flawed test design, not a target bug. Its **injection case's first live run was inconclusive, not a confirmed result** — the pipeline failed to resolve the ticker for "TCS" (a very well-known company) before ever reaching the poisoned tool response, so "no canary leaked" reflects the attack surface never being exercised, not resistance. A clean rerun is needed for a real verdict; see [docs/architecture.md](docs/architecture.md) for the full trace.
>
...
- [x] **Phase 2** — Faithfulness scorer (RAGAS-style LLM-judge, **100% calibration agreement**) + injection-resistance scorer (deterministic canary + judge fallback) + one injection test case per adapter (a poisoned RAG document, a monkeypatched ADK tool response — mechanism revised from the original MCP-server-mock plan per the MCP-bypass finding). **RAG chatbot injection case: confirmed RESISTED**, live-verified. **ADK injection case: first live run inconclusive** — the pipeline failed to resolve the ticker before reaching the poisoned tool response, so the attack was never actually exercised; needs a clean rerun for a real verdict (tight 5 req/day quota on this key, so not retried automatically — see docs/architecture.md).
